...
TFSException::TFSException(const std::filesystem::path::iterator &begin,
    const std::filesystem::path::iterator &end,
    NFSExceptionType type)
    : m_xType{type} {
...
TFSException::TFSException(const std::filesystem::path& path, NFSExceptionType type) {
    UpdateMessage(path.c_str(), type);
}
